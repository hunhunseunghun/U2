...
function Tutorial(props) {
  ...
  const toolChange = idx => {
    ...
    tempToolList.map((tl, index) => {
      return index !== tempToolList.length - 1
        ? (toolStr += tl + '-')
        : (toolStr += tl);
    });
    axios
    ...
    <div className={'contents_wrap'}>
      <div className={'filter_section'}>
        <div className={'ft_deco'}>
          <div
            className={
              'ft_title mobile_view ' + (mobileFilterViewYn ? 'active' : '')
            }
            onClick={() => {
        ...
        {(!mobileYn || mobileFilterViewYn) && (
          <div
            className={'sub_filter_section'}
            onClick={() => {
          ...
          >
            <div className={'ft_process'}>
              {categoryList.map((cItem, cIndex) => (
                <div
                  className={
                    'ftp_item ' +
                    (cItem.categoryIdx === currentCate ? 'active' : '')
                  }
                  key={cIndex}
                  onClick={e => {
                    cateChange(cItem.categoryIdx);
                  }}
                >
                  <span className={'ftp_num'} title={cItem.categoryName}>
                    {cIndex + 1}
                  </span>
                  <span className={'ftp_t'}>. {cItem.categoryName}</span>
                  {cIndex !== categoryList.length - 1 && (
            ...
            <div className={'ft_tags'}>
              {tagList.map((tagItem, tIndex) => (
                <div
                  key={tIndex}
                  className={
                    'tag_item ' + (tagItem === currentTag ? 'active' : '')
                  }
                  onClick={() => {
                    tagChange(tagItem);
                  }}
              ...
              <ul>
                {toolList.map((toolItem, tIndex) => (
                  <li
                    ...
                    className={
                      currentTool.indexOf(toolItem.toolIdx) >= 0 ? 'active' : ''
                    }
                  >
                    {toolItem.toolName}
                  </li>
        ...
        <div className={'ft_right_section'}>
          <div className={'ftr_select_item'}>
            <div
              className={'ftr_selected ' + (cateOrderPopYn ? 'active' : '')}
              onClick={e => {
                e.stopPropagation();
                setCateProgramPopYn(false);
                setCateOrderPopYn(!cateOrderPopYn);
              }}
            >
              {currentSort === 1
                ? '최신순'
          ...
          <div className={'ftr_select_item mobile_hidden'}>
            <div
              className={'ftr_selected ' + (cateProgramPopYn ? 'active' : '')}
              onClick={e => {
                e.stopPropagation();
                setCateOrderPopYn(false);
            ...
            >
              프로그램 <img src={'/img/ic_arrow_down.svg'} />
            </div>
            {cateProgramPopYn && (
              <div className={'pop_sub ftr_pop ftr_pop_program'}>
                <ul>
                  {toolList.map((toolItem, tIndex) => (
                    <li
                      key={tIndex}
                    ...
                    >
                      {toolItem.toolName}
                      {currentTool.indexOf(toolItem.toolIdx) >= 0 && (
                        <span className={'active_deco'}></span>
                      )}
                    </li>
                  ))}
                </ul>
      ...
      <div className={'tutorial_items'}>
                ...
                <Link to={'/tutorial/detail/' + tItem.topicIdx} key={index}>
                  <div className={'tt_item'}>
                    ...
                    <div className={'tt_contents'}>
                      <div className={'tt_title'} title={tItem.topicName}>
                        {tItem.topicName}
                      </div>
                      <div className={'tt_sub'}>
                        {tItem.categoryName}/ 총 {tItem.timeTotal}시간/{' '}
        ...
        {tutorialItems.length === 0 && (
          <div className={'not_contents'}>해당하는 콘텐츠가 없습니다.</div>
        )}
      </div>
    </div>
  );
