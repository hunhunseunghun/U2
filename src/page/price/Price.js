...
function Tutorial(props){
        ...
        setPriceList1(priceList.filter((p_item)=>{
         ...
         if(currentPrice){
             if(currentPrice===10000){
                 ...
                 if(p_item.price>=10000&&p_item.price<=currentPrice) {  console.log(p_item.price); priceFilterYn=true;}
    ...
    return(
        <div className={'contents_wrap'}>
            {comparePopYn&&<div className={'popup_section'} onClick={()=>{allPopupClose()}}>
                {comparePopYn&&<div className={'popup pop_price_compare'} onClick={(e)=>{e.stopPropagation();priceListPopClose();}}>
                    <div className={'pop_title'}>요금제 비교하기</div>
                    <div className={'pop_x_bt'} onClick={()=>{allPopupClose()}}><img src={'/img/ic_x_bt.svg'}/></div>
                    <div className={'pop_contents'}>
                        <div className={'pc_items'}>
                            {priceList.filter((pi)=>(compareItems.indexOf(pi.chargeIdx)>=0)).map((pcItem,pcIndex)=><div className={'price_item disable_item'}>
                                ...
                                <div className={'price_info_items'}>
                                    <div className={'pi_item'}>
                                        ...
                                        <div className={'pi_contents'}>{pcItem.tools.length===0?'없음':pcItem.tools.map((tl,tl_index)=>{
                                    ...
                                    <div className={'pi_item'}>
                                        ...
                                        <div className={'pi_contents'}>
                                            {pcItem.storageMasters.length===0?'없음':pcItem.storageMasters.map((st,stIndex)=><div key={stIndex}>{st.storageName+' 공간 '+st.storageSize+'gb'}</div>)}
                                        </div>
                                    </div>
                                    <div className={'pi_item'}>
                                    ...
                                    <div className={'pi_item'}>
                                        ...
                                        <div className={'pi_contents'}>{pcItem.toolsAdditional.length===0?'없음':pcItem.toolsAdditional.map((tl,tl_index)=>{
                                            if((tl_index+1)!==pcItem.tools.length) return tl.toolName+',';
                                            else return tl.toolName;
                                        })}</div>
                                    </div>
                                    <div className={'pi_item'}>
                                        ...
                                        <div className={'pi_contents'}>
                                            {pcItem.storageMastersAdditional.length===0?'없음':pcItem.storageMastersAdditional.map((st,stIndex)=><div key={stIndex}>{st.storageName+' 공간 '+st.storageSize+'gb'}</div>)}
                                        </div>
                                    </div>
                                    <div className={'pi_item'}>
                                ...
                                <div className={'price_detail'} dangerouslySetInnerHTML={{__html:brTagParser(pcItem.description)}}></div>
                                <div className={'price_bt_section'}>
                                    <div className={'price_info_section'}>
                                        <div className={'pr_select_item'}>
                                            <div className={'pr_selected'} onClick={(e)=>{e.stopPropagation();disCountPopOpen(pcItem);}}>{pcItem.disCountName}시 <img src={'/img/ic_arrow_down.svg'}/></div>
                                            {pcItem.disCountPopYn&&<div className={'pop_sub ftr_pop ftr_price_program'}>
                                                <ul>
                                                    {priceDiscounts.map((d_item,d_index)=><li key={d_index} onClick={(e)=>{e.stopPropagation();selectDiscount(pcItem,d_item)}}>{d_item.codeDesc}</li>)}
                                                </ul>
                                            </div>}
                                        </div>
                                        <div className={'pr_value'}>월 {(pcItem.price*(1-pcItem.disCount*0.01)).toLocaleString()}원</div>
                                    </div>
            ...
            {compareAlertPopYn&&<div className={'popup_section no_back'} onClick={()=>{allPopupClose()}}>
                {compareAlertPopYn&&<div className={'popup pop_price_compare_error'}>
                    ...
                    <div className={'pop_bt_section'}>
                        <button className={'default_bt pop_check_bt'} onClick={()=>{allPopupClose()}}>확인</button>
                    </div>
                </div>}
            </div>}
            <div className={'filter_section'}>
               <div className={'ft_deco'}>
                ...
                {mobileFilterViewYn&&<div className={'sub_filter_section ft_price'} onClick={()=>{setMobileFilterViewYn(false)}}>
                    <div className={'ft_title'}>요금제구성</div>
                    <ul>
                        <li className={currentType===1?'active':''} onClick={(e)=>{setCurrentType(currentType!==1?1:0)}}>정기요금제</li>
                        <li className={currentType===2?'active':''} onClick={(e)=>{setCurrentType(currentType!==2?2:0)}}>부가서비스</li>
                    </ul>
                    <div className={'ft_title'}>영상편집SW</div>
                    <ul>
                        {toolList.map((toolItem,tIndex)=><li className={currentTool.indexOf(toolItem.toolIdx)>=0?'active':''} key={tIndex} onClick={()=>{toolChange(toolItem.toolIdx)}}>{toolItem.toolName}</li>)}
                    </ul>
                    <div className={'ft_title'}>스토리지</div>

                        <ul>
                            {cateStorages.map((st,stIndex)=><li className={currentStorage.indexOf(st.storageIdx)>=0?'active':''} key={stIndex} onClick={()=>{storageChange(st.storageIdx)}}>{st.storageName+' 공간 '+st.storageSize+'gb'}</li>)}
                        </ul>
                    ...
                    <div className={'ft_title'}>요금제가격</div>
                        ...
                        <ul>
                            <li className={currentPrice===10000?'active':''} onClick={(e)=>{setCurrentPrice(currentPrice!==10000?10000:0)}}>~10,000원 미만</li>
                            <li className={currentPrice===50000?'active':''} onClick={(e)=>{setCurrentPrice(currentPrice!==50000?50000:0)}}>10,000~50,000원</li>
                            <li className={currentPrice===100000?'active':''} onClick={(e)=>{setCurrentPrice(currentPrice!==100000?100000:0)}}>50,000원~100,000원</li>
                            <li className={currentPrice===100001?'active':''} onClick={(e)=>{setCurrentPrice(currentPrice!==100001?100001:0)}}>10만원 초과</li>
                        </ul>

                </div>}
                {!mobileYn&&<div className={'fr_left_section'}>
                    <div className={'ftr_select_item'}>
                        <div className={'ftr_selected'} onClick={(e)=>{e.stopPropagation();allCatePopupClose();setCateTypePopYn(!cateTypePopYn)}}>요금제구성 <img src={'/img/ic_arrow_down.svg'}/></div>
                        {cateTypePopYn&&<div className={'pop_sub ftr_pop ftr_price_cate'}>
                            <ul>
                               <li onClick={(e)=>{setCurrentType(currentType!==1?1:0)}}>정기요금제{currentType===1&&<span className={'active_deco'}></span>}</li>
                               <li onClick={(e)=>{setCurrentType(currentType!==2?2:0)}}>부가서비스{currentType===2&&<span className={'active_deco'}></span>}</li>
                                </ul>
                        </div>}
                    </div>
                    <div className={'ftr_select_item'}>
                        <div className={'ftr_selected'} onClick={(e)=>{e.stopPropagation();allCatePopupClose();setCateProgramPopYn(!cateProgramPopYn)}}>영상편집SW <img src={'/img/ic_arrow_down.svg'}/></div>
                    ...
                    <div className={'ftr_select_item'}>
                        <div className={'ftr_selected'} onClick={(e)=>{e.stopPropagation();allCatePopupClose();setCateStoragePopYn(!cateStoragePopYn)}}>스토리지 <img src={'/img/ic_arrow_down.svg'}/></div>
                        {cateStoragePopYn&&<div className={'pop_sub ftr_pop ftr_price_storage'}>
                            <ul>
                                {cateStorages.map((st,stIndex)=><li key={stIndex} onClick={()=>{storageChange(st.storageIdx)}}>{st.storageName+' 공간 '+st.storageSize+'gb'}{currentStorage.indexOf(st.storageIdx)>=0&&<span className={'active_deco'}></span>}</li>)}
                            </ul>
                        </div>}
                    </div>
                    <div className={'ftr_select_item'}>
                        <div className={'ftr_selected'} onClick={(e)=>{e.stopPropagation();allCatePopupClose();setCateUseTimePopYn(!cateUseTimePopYn)}}>최대사용시간 <img src={'/img/ic_arrow_down.svg'}/></div>
                        {cateUseTimePopYn&&<div className={'pop_sub ftr_pop ftr_price_use_time'}>
                            <ul>
                                <li onClick={(e)=>{setCurrentUseTime(currentUseTime!==1?1:0)}}>1시간 이하{currentUseTime===1&&<span className={'active_deco'}></span>}</li>
                                <li onClick={(e)=>{setCurrentUseTime(currentUseTime!==35?35:0)}}>35시간 이하{currentUseTime===35&&<span className={'active_deco'}></span>}</li>
                                <li onClick={(e)=>{setCurrentUseTime(currentUseTime!==60?60:0)}}>60시간 이상{currentUseTime===60&&<span className={'active_deco'}></span>}</li>
                            </ul>

                        </div>}
                    </div>
                    <div className={'ftr_select_item'}>
                        <div className={'ftr_selected'} onClick={(e)=>{e.stopPropagation();allCatePopupClose();setCatePricePopYn(!catePricePopYn)}}>요금제가격 <img src={'/img/ic_arrow_down.svg'}/></div>
                        {catePricePopYn&&<div className={'pop_sub ftr_pop ftr_price_price'}>
                            <ul>
                                <li onClick={(e)=>{setCurrentPrice(currentPrice!==10000?10000:0)}}>~10,000원 미만{currentPrice===10000&&<span className={'active_deco'}></span>}</li>
                                <li onClick={(e)=>{setCurrentPrice(currentPrice!==50000?50000:0)}}>10,000~50,000원{currentPrice===50000&&<span className={'active_deco'}></span>}</li>
                                <li onClick={(e)=>{setCurrentPrice(currentPrice!==100000?100000:0)}}>50,000원~100,000원{currentPrice===100000&&<span className={'active_deco'}></span>}</li>
                                <li onClick={(e)=>{setCurrentPrice(currentPrice!==100001?100001:0)}}>10만원 초과{currentPrice===100001&&<span className={'active_deco'}></span>}</li>
                            </ul>
                        </div>}
                    </div>
                </div>}
                <div className={'ft_right_section'}>
                    <div className={'ftr_select_item'}>
                        <div className={'ftr_selected '+(cateSortPopYn?'active':'')} onClick={(e)=>{e.stopPropagation();setCateSortPopYn(!cateSortPopYn)}}>{currentSort===1?'최신순':'오래된순'} <img src={'/img/ic_arrow_down.svg'}/></div>
                        {cateSortPopYn&&<div className={'pop_sub ftr_pop '}>
                            <ul>
                                <li onClick={(e)=>{sortChange(1)}}>최신순</li>
                                <li onClick={(e)=>{sortChange(0)}}>오래된순</li>
                            </ul>
                        </div>}
                    </div>
                </div>
            ...
            <div className={'price_section'}>
                {currentType!==2&&<div className={'price_items'}>
                    ...
                    {priceList1.filter((p_item,p_index)=>p_index<priceCompo1Count).map((pr,p_index)=><div className={'price_item disable_item '+(compareItems.indexOf(pr.chargeIdx)>=0?'compare_selected':'')} key={p_index}>
                        <div className={'price_title'}>{pr.chargeName}</div>
                        <div className={'price_desc'}>{pr.chargeSubName}</div>
                        <div className={'price_info_items'}>
                            <div className={'pi_item'}>
                                <div className={'pi_title'}>기본 SW</div>
                                <div className={'pi_contents'}>{pr.tools.length===0?'없음':pr.tools.map((tl,tl_index)=>{
                            ...
                            <div className={'pi_item'}>
                                <div className={'pi_title'}>기본 스토리지</div>
                                <div className={'pi_contents'}>
                                    {pr.storageMasters.length===0?'없음':pr.storageMasters.map((st,stIndex)=><div key={stIndex}>{st.storageName+' 공간 '+st.storageSize+'gb'}</div>)}
                                </div>
                            </div>
                            <div className={'pi_item'}>
                                <div className={'pi_title'}>기본 사용시간</div>
                            ...
                            <div className={'pi_item'}>
                                <div className={'pi_title'}>추가 SW</div>
                                <div className={'pi_contents'}>{pr.toolsAdditional.length===0?'없음':pr.toolsAdditional.map((tl,tl_index)=>{
                                    if((tl_index+1)!==pr.tools.length) return tl.toolName+',';
                                    else return tl.toolName;
                                })}</div>
                            </div>
                            <div className={'pi_item'}>
                                <div className={'pi_title'}>추가 스토리지</div>
                                <div className={'pi_contents'}>
                                    {pr.storageMastersAdditional.length===0?'없음':pr.storageMastersAdditional.map((st,stIndex)=><div key={stIndex}>{st.storageName+' 공간 '+st.storageSize+'gb'}</div>)}
                                </div>
                            </div>
                            <div className={'pi_item'}>
                                <div className={'pi_title'}>추가 사용시간</div>
                        ...
                        <div className={'price_bt_section'}>
                            <button className={'default_bt price_sub_bt compare_bt'} onClick={()=>{compareItemClick((pr))}}>비교하기</button>
                            <div className={'share_bt_section'}>
                                <button className={'default_bt price_sub_bt'} onClick={(e)=>{e.stopPropagation();sharePopView(pr)}}>공유하기</button>
                            ...
                            <Link to={'/price/detail/'+pr.chargeIdx}><button className={'default_bt price_sub_bt'}>자세히보기</button></Link>
                            <div className={'price_info_section'}>
                                <div className={'pr_select_item'}>
                                    <div className={'pr_selected'} onClick={(e)=>{e.stopPropagation();disCountPopOpen(pr);}}>{pr.disCountName}시 <img src={'/img/ic_arrow_down.svg'}/></div>
                                    {pr.disCountPopYn&&<div className={'pop_sub ftr_pop ftr_price_program'}>
                                        <ul>
                                            {priceDiscounts.map((d_item,d_index)=><li key={d_index} onClick={()=>{selectDiscount(pr,d_item)}}>{d_item.codeDesc}</li>)}
                                        </ul>
                                    </div>}
                                </div>
                                <div className={'pr_value'}>월 {(pr.price*(1-pr.disCount*0.01)).toLocaleString()}원</div>
                            </div>
                            <button className={'default_bt price_buy_bt'}>구매하기</button>
                        </div>

                    </div>)}
                    {priceList.length!==0&&priceList1.length===0&&<div className={'no_contents'}>조건에 맞는 정기요금제가 없습니다.</div> }
                    {priceCompo1Count!==999&&priceList1.length>priceCompo1Count&&<div className={'price_bt_section'}>
                        <button className={'default_bt more_item_bt'} onClick={()=>{setPriceCompo1Count(999)}}>정기요금제 더보기</button>
                    </div>}
                </div>}

                {currentType!==1&&<div className={'price_items'}>
                    <div className={'section_title'}>부가서비스</div>
                    {priceList2.filter((p_item,p_index)=>p_index<priceCompo2Count).map((pr,p_index)=><div className={'price_item disable_item service_item'} key={p_index}>
                        <div className={'price_title'}>{pr.chargeName} <img src={'/img/'+pr.tools[0].toolLogo+'.png'}/></div>
                        <div className={'price_desc'}>{pr.chargeSubName}</div>
                        <div className={'price_detail'} dangerouslySetInnerHTML={{__html:brTagParser(pr.description)}}></div>
                        <div className={'price_bt_section'}>
                            <div className={'price_info_section'}>
                                <div className={'pr_select_item'}>
                                    <div className={'pr_selected'} onClick={(e)=>{e.stopPropagation();disCountPopOpen(pr);}}>{pr.disCountName}시 <img src={'/img/ic_arrow_down.svg'}/></div>
                    ...
                    {priceCompo2Count!==999&&priceList2.length>priceCompo2Count&&<div className={'price_bt_section'}>
                        <button className={'default_bt more_item_bt'} onClick={()=>{setPriceCompo2Count(999)}}>부가서비스 더보기</button>
                    </div>}
                </div>}
                {priceList.length!==0&&priceList2.length===0&&<div className={'no_contents'}>조건에 맞는 부가서비스가 없습니다.</div> }
                ...
                {compareItems.length>0&&<div className={'price_compare_section'}>
                    <div className={'pc_container'}>
                        ...
                        <div className={'pc_bt_section'}>
                            <div className={'pc_items'}>
                                {priceList.filter((pi)=>(compareItems.indexOf(pi.chargeIdx)>=0)).map((pcItem,pcIndex)=><div className={'pc_item'}>{pcItem.chargeName}</div>)}
                            </div>
                            <div className={'pc_right_bt_section'}>
                                <button className={'default_bt pc_reset_bt'} onClick={()=>{setCompareItems([])}}>선택 리셋하기</button>
                                <button className={'default_bt price_compare_bt '+(compareItems.length<2?'disable_bt':'')} onClick={()=>{comparePriceView()}}>비교하기</button>
                            </div>
                        </div>
                    </div>
                </div>}
